...
export default function Portal(){
 ...
 return <div className="min-h-screen grain-bg overflow-hidden"><SiteHeader/>
  ...
  <section id="categorias" className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24 scroll-mt-24">
   ...
   {!loading && !failed && categories.some(c => (c.materials || []).length > 0) && <div className="premium-surface rounded-2xl p-5 sm:p-6 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><p className="text-xs font-bold text-purple-600 uppercase tracking-wider">Pedido em lote</p><h3 className="font-display text-xl font-bold text-slate-900 mt-1">Todos os materiais em um único kit</h3><p className="text-sm text-slate-500 mt-1">Combine sub-itens de várias categorias, escolha as quantidades e revise antes de enviar.</p></div><Button onClick={() => navigate('/kit')} className="bg-[#660099] hover:bg-[#520080] gap-2 shrink-0">Montar kit de materiais <ArrowRight size={16}/></Button></div>}
